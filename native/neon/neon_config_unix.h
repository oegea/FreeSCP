/* Native (macOS/Linux) neon config — generated by libs/neon/configure (autotools).
   Captured into the repo so the build is reproducible without re-running configure.
   Included from libs/neon/src/config.h under #ifndef _WIN32 (guarded; see UPSTREAM-PATCHES).
   Regenerate: out-of-tree 'sh libs/neon/configure --with-ssl=openssl --with-expat ...' then
   'sh ./config.status config.h'. See STATUS.md 'Phase 4 neon build'. */
#ifndef WINSCP_NEON_CONFIG_UNIX_H
#define WINSCP_NEON_CONFIG_UNIX_H
...
#define PACKAGE_STRING "neon 0.34.2"
