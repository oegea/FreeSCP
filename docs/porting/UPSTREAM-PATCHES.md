...
- **source/core/SecureShell.h** — `typedef UINT_PTR SOCKET;` wrapped in `#ifdef _WIN32`; on
  non-Windows SOCKET is PuTTY's `int` fd (from platform.h). Avoids the engine↔putty SOCKET
  type clash.
